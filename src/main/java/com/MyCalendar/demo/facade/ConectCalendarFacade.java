package com.MyCalendar.demo.facade;
...
public class ConectCalendarFacade {
	...
	private static final List<String> SCOPES = Arrays.asList(CalendarScopes.CALENDAR, CALENDAR_EVENTS,
			TasksScopes.TASKS);
	...
	public static Calendar getCalendarService() {
		try {
			...
			return new Calendar.Builder(http, JSON_FACTORY, getCredentials(http)).setApplicationName(APPLICATION_NAME)
					.build();
		} catch (GeneralSecurityException e) {
	...
	public static Tasks getTasksService() {
		try {
			InputStream inputStream = ConectCalendarFacade.class.getResourceAsStream(CREDENTIALS_FILE_PATH);
			HttpTransport httpTransport = com.google.api.client.googleapis.javanet.GoogleNetHttpTransport
					.newTrustedTransport();
			return new Tasks.Builder(httpTransport, JSON_FACTORY, getCredentials((NetHttpTransport) httpTransport))
